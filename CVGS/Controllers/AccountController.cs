...
namespace CVGS.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var result = await SignInManager.PasswordSignInAsync(model.DisplayName, model.Password, model.RememberMe, shouldLockout: true);
            switch (result)
            {
                case SignInStatus.Success:
                    var currentUser = await UserManager.FindByNameAsync(model.DisplayName);
                    if (currentUser != null)
                    {
                        Session["cart"] = _db.UserCartItems.Where(i => i.UserId == currentUser.Id).Count().ToString();
                    }
